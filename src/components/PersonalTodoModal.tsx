...
export default function PersonalTodoModal({
  targetUserId,
  targetUserName,
  targetUserColor,
  targetUserEmoji,
  ...
  useEffect(() => {
    const unsub = subscribePersonalTodoList(targetUserId, groupId, (fetched) => {
      if (fetched) {
        ...
        setList({ userId: targetUserId, groupId, categories: [] });
      }
      setLoading(false);
    });
    return () => unsub();
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-50 rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
        {/* 헤더 */}
        <div className="bg-white px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div
              className="w-9 h-9 rounded-2xl flex items-center justify-center text-white font-bold shadow-sm text-base"
            ...
            <div className="flex gap-4 p-5 h-full items-start">
              ...
              {(list?.categories ?? []).map((cat) => {
                ...
                return (
                  <div
                    key={cat.id}
                    className="w-64 shrink-0 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col max-h-[calc(90vh-140px)]"
                  >
                    {/* 카테고리 헤더 */}
                        ...
                        >
                          {editingItemId === item.id && isOwner ? (
                            ...
                            <div className="flex items-start gap-2">
                              {/* 체크박스 */}
                              <button
                                onClick={() => isOwner && toggleItem(cat.id, item.id)}
                                disabled={!isOwner}
              ...
              {isOwner && (
                <div className="w-64 shrink-0">
                  {showAddCat ? (
                    <div className="bg-white rounded-2xl border border-sky-200 shadow-sm px-4 py-3">
                      <input
                        ref={newCatRef}
                        autoFocus
                        value={newCatName}
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl border-2 border-dashed border-slate-200 text-slate-400 hover:border-sky-300 hover:text-sky-400 transition-all text-sm font-semibold"
                    >
                      <Plus size={15} />
                      새 카테고리
                    </button>
              ...
              {/* 카테고리 없을 때 빈 상태 */}
              {(list?.categories ?? []).length === 0 && !isOwner && (
